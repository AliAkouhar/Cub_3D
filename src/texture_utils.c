...
{
    return ((int)(wall_x * (cub->textures[tex_index].texture_width / cub->tile_map)));
}

int get_texture_y(t_cub *cub, int y, int tex_index)
{
    return (((y - cub->wall_top_pixel) * cub->textures[tex_index].texture_height)/ cub->wall_strip_height);
}
...
unsigned int get_texture_color(t_cub *cub, int tex_index)
{
    ...
    color = *(unsigned int *)(cub->textures[tex_index].addr + (cub->texture_y * cub->textures[tex_index].line_length +
					cub->texture_x * (cub->textures[tex_index].b_per_pixel / 8)));
    return (color);
}
