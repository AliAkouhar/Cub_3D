...
t_point horizontal_intersection(t_cub *cub, t_ray ray)
{
    ...
    {
        if (is_a_wall(cub, inter, 'h', ray)) // door check if you hit door and set flag
            return (inter);
...
void cast_all_rays(t_cub *cub)
{
    ...
    {
        cub->is_ray_hit_door = 0;
        normalizing(cub);
        cub->ray_id = i;
        endpoint = cast(cub);
        if (cub->map_content[(int)(endpoint.y/ cub->tile_map)][(int)(endpoint.x / cub->tile_map)] == 'D')
        {
            printf("here\n");
            cub->is_ray_hit_door = 1;
        }
