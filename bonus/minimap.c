...
{
    cub->start.x = cub->player.point.x / cub->tile_map - RANGE_MINIMAP / cub->tile_map;
    cub->start.y = cub->player.point.y / cub->tile_map - RANGE_MINIMAP / cub->tile_map;
    if (cub->start.x < 0)
        cub->start.x = 0;
    if (cub->start.y < 0)
        cub->start.y = 0;
...
void    render_mini_map(t_cub *cub)
{
    ...
    render_player_square(cub, ((cub->player.point.x / cub->tile_map - cub->start.x) * MINI_TILE),
                    ((cub->player.point.y / cub->tile_map - cub->start.y) * MINI_TILE),
                    0xFFE4B5);
}
